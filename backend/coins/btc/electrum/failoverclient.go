...
func (f *failoverClient) HeadersSubscribe(
	setupAndTeardown func() func(error), result func(header *types.Header)) {
	failover.Subscribe(
		f.failover,
		func(c *client, result func(*types.Header, error)) {
			c.HeadersSubscribe(setupAndTeardown, func(header *types.Header, err error) {
				result(header, err)
			})
		},
...
func (f *failoverClient) ScriptHashSubscribe(
	...
	failover.Subscribe(
		...
		func(c *client, result func(string, error)) {
			c.ScriptHashSubscribe(setupAndTeardown, scriptHashHex, func(status string, err error) {
				result(status, err)
			})
		},
