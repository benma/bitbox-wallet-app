...
// TokenActive returns true if this token is configured to be active.
// code is the token id, e.g. "eth-erc20-usdt".
func (account Account) TokenActive(code string) bool {
	for _, tokenCode := range account.ActiveTokens {
		if tokenCode == code {
			return true
		}
	}
