...
const (
	...
	// Nothing is happeening, we are waiting for an AOPP request.
	aoppStateInactive aoppState = ""
...
// AOPPCancel resets the aopp state.b
...
// accounts to choose from. `accountsAndKeystoreLock` must be held when calling this function.
func (backend *Backend) aoppKeystoreRegistered() {
	...
	if !backend.keystore.CanSignMessage(backend.aopp.coinCode) {
		backend.aoppSetError(errAoppCannotSign)
		return
	}
...
func (backend *Backend) handleAOPP(uri *url.URL) {
	...
	coinCode, ok := map[string]coinpkg.Code{
		"btc": coinpkg.CodeBTC,
		// TODO: add support for ETH
		// "eth": coinpkg.CodeETH,
	}[strings.ToLower(q.Get("asset"))]
	if !ok {
...
func (backend *Backend) AOPPChooseAccount(code accounts.Code) {
	...
	account := func() accounts.Interface {
		for _, acct := range backend.accounts {
