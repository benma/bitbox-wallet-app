...
interface LoadedReceiveProps {
    receiveAddresses: Array<{ addresses: Array<{ addressID: string, address: string }> }>;
    secureOutput: {
...
class Receive extends Component<Props, State> {
    ...
    private verifyAddress = (addressesIndex: number) => {
        ...
        apiPost('account/' + this.props.code + '/verify-address', receiveAddresses[addressesIndex].addresses[activeIndex].addressID).then(() => {
    ...
    public render(
        ...
        let address = currentAddresses.addresses[activeIndex].address;
        ...
        const content = (
            <div style="position: relative;">
                ...
                <div class={['flex flex-row flex-between flex-items-center', style.labels].join(' ')}>
                    {
                        currentAddresses.addresses.length > 1 && (
                            <a
                                href="#"
                    ...
                    <p class={style.label}>{t('receive.label')} {currentAddresses.addresses.length > 1 ? `(${activeIndex + 1}/${currentAddresses.addresses.length})` : ''}</p>
                    {
                        currentAddresses.addresses.length > 1 && (
                            <a
                                href="#"
                                className={['flex flex-row flex-items-center', verifying || activeIndex >= currentAddresses.addresses.length - 1 ? style.disabled : '', style.next].join(' ')}
                                onClick={e => this.next(e, currentAddresses.addresses.length)}>
                                {/* {t('button.next')} */}
                                <svg
                                    className={[style.arrow, verifying ? style.disabled : ''].join(' ')}
                                    xmlns="http://www.w3.org/2000/svg"
        ...
        return (
            <div class="contentWithGuide">
                ...
                <Guide>
                    ...
                    {currentAddresses.addresses.length > 1 && <Entry key="guide.receive.whyMany" entry={t('guide.receive.whyMany')} />}
                    {currentAddresses.addresses.length > 1 && <Entry key="guide.receive.why20" entry={t('guide.receive.why20')} />}
                    {currentAddresses.addresses.length > 1 && <Entry key="guide.receive.addressChange" entry={t('guide.receive.addressChange')} />}
                </Guide>
            </div>
        );
    }
